...
import random 
import string
### se impprta un modelo de Django
...
def inicio(request):
    return render(request, "frmIniciarSeccion.html")
...
def inicioAdministrador(request):
    if request.user.is_authenticated:
        ...
        return render(request, "frmIniciarSeccion.html", {"mensaje": mensaje})
...
def listarEmpleadosTecnicos(request):
    if request.user.is_authenticated:
        try:
            mensaje =""
            #Esta es la consulta pata obtener los datos de los empleados cpn el rol de tecnico para ser agnidados desde el administrador
            tecnicos = User.objects.filter(groups__name__in=['Tecnico'])
        except Error as error:
            mensaje = str(error)
...
def asignarTecnicoCaso(request):
    if request.user.is_authenticated:
        try:
            idTecnico = int(request.POST['cdTecnico'])
            ...
            caso.save()
            #aqui se envia el correo al tecnico
            asunto = 'Registro Solicitud - Mesa de Servicio'
            mensajeCorreo = f'Cordial saludo, <b>{userTecnico.first_name} {userTecnico.last_name}</b>, nos permitimos \
                informarle que se le ha asignado un caso para darle la solucon a si problema, codigo de casp: \
                <b>{caso.casCodigo}</b>. <br><br> Se soliita que se atienda de manera oportuna, \
                según los acuerdos de solución establecidos para la Mesa de Servicios del CTPI-CAUCA.\
                <br><br>Lo invitamos a ingresar al sistema para gestipn ar su caso asigandos en la sigueiente url:\
                http://mesadeservicioctpicauca.sena.edu.co.'
            # crear el hilo para el envío del correo
            thread = threading.Thread(
            ...
            # ejecutar el hilo
            thread.start()
            mensaje = "Se ha asignado el caso al tecnico"
        except Error as error:
            mensaje = str(error)
    ...
    if request.user.is_authenticated:
        try:
            if transaction.atomic():
                ...
                solucionCaso.save()
                #actualiazr stado de caso dependiendo el tipo de la solucion
                if(tipoSolucion=="Definiiva"):
                    caso.casEstado=="Finalizada"
                    caso.save()

                    #se crea el objeto paara el tipo dle procedimeinto
                ...
                mensajeCorreo = f'Cordial saludo, <b>{userEmpleado.first_name} {userEmpleado.last_name}</b>, nos permitimos \
                    informarle que se le ha solucionado de tipo {tipoSolucion} al caso identificado con el codigo : \
                    <b>{caso.casCodigo}</b>. <br><br> Lo invitamos a revisar el equipo y verificar si est en su completo funcionamiento \
                    según los acuerdos de solución establecidos para la Mesa de Servicios del CTPI-CAUCA.\
                    <br><br>Lo invitamos a ingresar al sistema para gestipn ar su caso asigandos en la sigueiente url:\
                    http://mesadeservicioctpicauca.sena.edu.co.'
            # crear el hilo para el envío del correo
